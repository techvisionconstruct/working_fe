...
export function FormulaBuilder({
  ...
  return (
    <div className="space-y-3">
      <div
        className={`border rounded-xl p-4 flex flex-wrap gap-2 min-h-[70px] bg-gradient-to-br from-background/80 to-background/60 relative transition-all duration-200 ${
      ...
      >
        {validFormulaTokens.map((token) => (
          <Badge
            key={token.id}
            variant="outline"
            className={`gap-1.5 px-3 py-1.5 text-sm rounded-lg transition-all duration-200 hover:shadow-md cursor-text ${
              token.type === "variable"
        ...
        <Input
          ...
          className="border-0 shadow-none focus:outline-none focus:ring-0 p-0 h-10 text-sm flex-1 min-w-[120px] bg-transparent placeholder:text-muted-foreground/60"
        />

        {validationError && (
          <div className="absolute -bottom-7 left-0 text-xs text-red-500 flex items-center gap-1">
        ...
        {isFormulaValid && validFormulaTokens.length > 0 && (
          <div className="absolute -bottom-7 left-0 text-xs text-green-600 flex items-center gap-1">
