...
const AddTradeDialog: React.FC<AddTradeDialogProps> = ({
  ...
  // Internal state
  const [newTradeName, setNewTradeName] = useState(initialName);
  ...
  // Trade creation mutation
  const { mutate: createTradeMutation, isPending: isSubmitting } = useMutation({
    mutationFn: createTrade,
    onSuccess: (response) => {
      if (response && response.data) {
        const createdTrade = response.data;
        toast.success(`Trade "${createdTrade.name}" created successfully!`);
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-auto min-w-[600px] max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <DialogFooter className="justify-end">
          ...
          <Button onClick={handleAddTrade} disabled={isSubmitting} className="rounded-full">
            {isSubmitting ? (
              <>
