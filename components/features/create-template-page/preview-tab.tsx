...
import { ModuleForm, ParameterForm, previewSchema, TemplateDetailsForm } from "./zod-schema";
import React, { useState } from "react";
import { AlertCircle, Check, ArrowLeft, FileText, Package, Ruler } from "lucide-react";
    ...
    <Form {...form}>
      <div className="space-y-8">
        <div className="space-y-2">
          <h2 className="text-2xl font-semibold tracking-tight">Preview Template</h2>
          <p className="text-sm text-muted-foreground">
            Review your template details before submitting. Make sure everything looks correct.
          </p>
        </div>
            ...
            <CardContent className="p-0">
              {templateDetails.image && (
                <div className="relative h-48 md:h-64 w-full overflow-hidden">
                  <img
                    src={typeof templateDetails.image === "string" ? templateDetails.image : URL.createObjectURL(templateDetails.image)}
                    alt="Template"
              ...
              <div className="p-6">
                ...
                {form.formState.errors.description && (
                  <div className="text-destructive flex items-center gap-1 text-xs mt-1">
                    <AlertCircle className="w-3 h-3" />
                    {typeof form.formState.errors.description?.message === 'string'
                          ...
                          <div className="flex items-start justify-between">
                            ...
                            {module.elements && (
                              <div className="bg-primary/20 text-primary text-xs px-2 py-1 rounded-full font-medium">
                                {module.elements.length} {module.elements.length === 1 ? 'element' : 'elements'}
                              </div>
                            )}
                          </div>
                        ...
                        <div className="mt-4">
                          ...
                          <div className="grid gap-3">
                            {module.elements.map((el) => (
                              <div key={el.id} className="p-3 rounded-md bg-muted/40 hover:bg-muted/60 transition-colors">
                                <div className="flex items-center justify-between">
                                  <div className="font-medium text-sm">{el.name}</div>
                                </div>
                                <div className="text-xs text-muted-foreground mt-1">{el.description}</div>
                                
                                {(el.formula || el.labor_formula) && (
                                  <div className="grid grid-cols-2 gap-2 mt-2">
                                    {el.formula && (
                                      <div className="text-xs bg-background/80 p-2 rounded border">
                                        <span className="block font-medium mb-1">Formula</span>
                                        <code className="text-primary">{el.formula}</code>
                                      </div>
                                    )}
                                    {el.labor_formula && (
                                      <div className="text-xs bg-background/80 p-2 rounded border">
                                        <span className="block font-medium mb-1">Labor Formula</span>
                                        <code className="text-primary">{el.labor_formula}</code>
                                      </div>
                                    )}
                                  </div>
                                )}
                                ...
                                {el.markup && (
                                  <div className="mt-2 text-xs bg-background/80 p-2 rounded border">
                                    <span className="block font-medium mb-1">Markup</span>
                                    <code className="text-primary">{el.markup}</code>
                                  </div>
                                )}
                              </div>
                            ))}
