...
export function FormulaBuilder({
  ...
  const handleInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (suggestions.length > 0) {
      ...
      } else if (formulaInput.trim() && !variables.some(v => v.name.toLowerCase() === formulaInput.toLowerCase())) {
        ...
        const exactMatch = variables.find(v => v.name.toLowerCase() === formulaInput.trim().toLowerCase());
        if (exactMatch) {
  ...
  useEffect(() => {
    ...
    if (apiVariables?.data) {
      const apiMatches = (apiVariables.data as VariableResponse[])
        .filter(v => 
          v.name.toLowerCase().includes(formulaInput.toLowerCase()) &&
              ...
              formulaTokens.some(token => 
                ...
                <div className="p-3 cursor-pointer hover:bg-accent" 
                     onClick={() => addFormulaToken(
                       variables.find(v => v.name.toLowerCase() === formulaInput.trim().toLowerCase())?.name || 
                       formulaInput.trim(), 
                       'variable'
                     )}>
                  <div className="flex-1">
