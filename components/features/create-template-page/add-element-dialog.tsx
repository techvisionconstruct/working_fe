...
export function AddElementDialog({
  ...
  const handleMaterialFormulaInputKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      const shortcuts: Record<string, {text: string, type: 'operator' | 'function'}> = {
        '+': { text: '+', type: 'operator' },
        '-': { text: '-', type: 'operator' },
        '*': { text: '*', type: 'operator' },
        '/': { text: '/', type: 'operator' },
  ...
  // Add pendingLaborVariable state
  const [pendingMaterialVariable, setPendingMaterialVariable] = useState<string | null>(null);
  const [pendingLaborVariable, setPendingLaborVariable] = useState<string | null>(null);

  // Update useEffect to handle both pending variables
  React.useEffect(() => {
    if (pendingMaterialVariable && parameterValue.some(p => p.name === pendingMaterialVariable)) {
      addMaterialFormulaToken(pendingMaterialVariable, 'variable');
      setPendingMaterialVariable(null);
    }
    if (pendingLaborVariable && parameterValue.some(p => p.name === pendingLaborVariable)) {
      addLaborFormulaToken(pendingLaborVariable, 'variable');
      setPendingLaborVariable(null);
    }
  }, [parameterValue, pendingMaterialVariable, pendingLaborVariable]);
          ...
          <div className="grid gap-2">
            ...
            <div className="border rounded-md p-2 flex flex-wrap gap-2 min-h-[60px] bg-background relative">
              ...
              <div className="relative flex-1 min-w-[100px]">
                ...
                {/* Message to create new variable */}
                {showMaterialFormulaSuggestions && materialFormulaInput && materialFormulaSuggestions.length === 0 && (
