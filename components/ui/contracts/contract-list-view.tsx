  ...
  const filteredAndSortedContracts = useMemo(() => {
    ...
    const filtered = searchQuery
      ? contracts.filter((contract) =>
          ...
          (contract.contractDescription && contract.contractDescription.toLowerCase().includes(searchQuery.toLowerCase()))
        )
      : contracts;
    ...
    <div className="space-y-3">
      {filteredAndSortedContracts.length > 0 ? (
        filteredAndSortedContracts.map((contract) => (
          <Link key={contract.uuid} href={`contracts/${contract.uuid}`} className="block group">
            <motion.div
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="p-5 w-full">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {contract.contractModules?.slice(0, 3).map((category) => (
                      <Badge
                        key={category.id}
                        variant="outline"
                    ...
                    {contract.contractModules?.length > 3 && (
                      <Badge
                        variant="outline"
                        className="font-semibold uppercase text-xs rounded-lg border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-200 px-2 py-0.5"
                      >
                        +{contract.contractModules?.length - 3} more
                      </Badge>
                    )}
                  </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        <span className="ml-1 h-4 w-4 rounded bg-zinc-200 dark:bg-zinc-700 text-xs text-zinc-700 dark:text-zinc-200 flex items-center justify-center">
                          {contract.project_parameters?.length || 0}
                        </span>
                      </Badge>
                      <Badge
                        variant="outline"
                      ...
                      >
                        ...
                        <span className="ml-1 h-4 w-4 rounded bg-zinc-200 dark:bg-zinc-700 text-xs text-zinc-700 dark:text-zinc-200 flex items-center justify-center">
                          {contract.project_modules?.length || 0}
                        </span>
                      </Badge>
                    </div>
